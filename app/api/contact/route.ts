  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from,
        to: [contactEmail],
        subject: "Novo contato pelo site — Rafael Setubal",
        ...(emailPattern.test(contact) ? { reply_to: contact } : {}),
        text: `Nome: ${name}\nEmpresa: ${company}\nContato: ${contact}\nInteresse: ${body.necessidades && Array.isArray(body.necessidades) && body.necessidades.length > 0 ? body.necessidades.join(", ") : "Não informado"}\n\n${description || "(Sem mensagem adicional)"}`
      }),
    ...
    if (!response.ok) {
      ...
      return fail("Não foi possível enviar. Tente novamente ou fale pelo WhatsApp.", 502)
    }
